...
Kohaku is the strategic supplement that sits one layer above [[ruflo-swarm-bridge]] and [[opendesign-router]]. It is not an executor. It is a router and a framer.
...
5. HAND OFF to executor:
   - 3+ files / big-project coding → Ruflo swarm
   - Design domain → OpenDesign router
   - Single-file / quick task → direct skill stack
...
- **Not a replacement for Ruflo.** Ruflo executes; Kohaku decides what to execute and why.
- **Not a replacement for /goal.** Kohaku wraps /goal — it does not bypass it.
- **Not a brainstorming skill.** Use `superpowers:brainstorming` first if requirements are unclear. Kohaku assumes you know the objective and need strategic framing.
- **Not a code reviewer.** For PR-time review, use `compound-engineering:ce-code-review`. Kohaku can inform that review but does not replace it.
